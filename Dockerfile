...
RUN git clone https://github.com/fengou/SoftEtherVPN.git
ENV USE_MUSL=YES
RUN cd SoftEtherVPN &&\
	git submodule init &&\
...
COPY --from=builder /usr/local/src/SoftEtherVPN/bin/vpnserver/vpnserver /usr/local/src/SoftEtherVPN/bin/vpncmd/vpncmd /usr/local/src/SoftEtherVPN/bin/vpnserver/hamcore.se2 ./
